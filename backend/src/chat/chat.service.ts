...
export class ChatService {
  private readonly socketUsers: Map<string, SocketUser> = new Map();
  ...
  async checkUserChannelAccess(
    ...
    console.log(
      `Checking channel access for user ${userId} to channel ${channelId}`
    );
    ...
    const hasGuildAccess = await this.checkUserGuildAccess(
      userId,
      channel.guild.id
    );
    console.log(
      `Guild access result for user ${userId} in guild ${channel.guild.id}: ${hasGuildAccess}`
    );
  ...
  async saveMessage(
    ...
    console.log(
      `Attempting to save message from user ${authorId} to channel ${channelId}`
    );
    ...
    if (!channel) {
      console.log(
        `Channel ${channelId} not found, attempting to find or create a default channel for user ${authorId}`
      );
      ...
      if (userGuilds.length === 0) {
        ...
        throw new ForbiddenException(
          "You do not have access to any guilds. Please create or join a guild first."
        );
      }
      ...
      console.log(
        `User has access to guild ${guild.id}, checking for existing channels`
      );
      ...
      if (existingChannels.length === 0) {
        console.log(
          `Guild ${guild.id} has no channels, creating default channel`
        );
        ...
        console.log(
          `Created default channel '${defaultChannel.name}' with ID: ${defaultChannel.id} for guild ${guild.id}`
        );
        ...
        console.log(
          `Using existing channel ${channel.id} (${channel.name}) from guild ${guild.id}`
        );
      }
    } else {
      ...
      if (!guild) {
        console.log(
          `Guild ${channel.guild.id} not found - channel ${channelId} is orphaned`
        );
    ...
    console.log(
      `Channel access check result for channel ${channel.id}: ${hasAccess}`
    );
    if (!hasAccess) {
      console.error(
        `Access denied for user ${authorId} to channel ${channel.id}`
      );
  ...
  async getChannelMessages(
    ...
    if (!hasAccess) {
      console.error(
        `Access denied for user ${userId} to channel ${channel.id}`
      );
      throw new ForbiddenException("You do not have access to this channel");
    }
  ...
  async getGuildMemberCount(guildId: string): Promise<number> {
    const memberCount = await this.userGuildRepository.count({
      guild: guildId,
    });
    return memberCount;
  }
  ...
  isUserOnline(userId: string): boolean {
    return Array.from(this.socketUsers.values()).some(
      (user) => user.id === userId
    );
  }
  ...
  getUserLastActivity(userId: string): Date | null {
    const socketUser = Array.from(this.socketUsers.values()).find(
      (user) => user.id === userId
    );
  ...
  async getGuild(guildId: string, userId: string): Promise<Guild> {
    ...
    if (guild.channels.length === 0) {
      console.log(
        `[DEBUG] Guild ${guildId} has no channels, creating default channel`
      );
      const defaultChannel = new Channel();
      defaultChannel.name = "general";
      defaultChannel.description = "General discussion channel";
      ...
      await this.channelRepository.persistAndFlush(defaultChannel);
      console.log(
        `[DEBUG] Created default channel '${defaultChannel.name}' with ID: ${defaultChannel.id} for existing guild ${guild.id}`
      );
  ...
  async createGuild(
    ...
    await this.channelRepository.persistAndFlush(defaultChannel);
    console.log(
      `[DEBUG] Created default channel '${defaultChannel.name}' with ID: ${defaultChannel.id} for guild ${guild.id}`
    );
  ...
  async updateGuild(
    ...
    if (
      !userGuild ||
      (userGuild.role !== UserGuildRole.OWNER &&
        userGuild.role !== UserGuildRole.ADMIN)
    ) {
      throw new ForbiddenException(
        "You do not have permission to update this guild"
      );
    }
  ...
  async getChannel(channelId: string, userId: string): Promise<Channel> {
    ...
    if (!channel) {
      console.log(
        `Channel ${channelId} not found, attempting to find a default channel for user ${userId}`
      );
  ...
  async createChannel(
    ...
    console.log(
      `[DEBUG] createChannel called - guildId: ${guildId}, name: ${name}, userId: ${userId}`
    );

    const hasAccess = await this.checkUserGuildAccess(userId, guildId);
    if (!hasAccess) {
      console.log(
        `[DEBUG] createChannel - User ${userId} does not have access to guild ${guildId}`
      );
      throw new ForbiddenException("You do not have access to this guild");
    }

    console.log(
      `[DEBUG] createChannel - User has access, finding guild ${guildId}`
    );
    const guild = await this.guildRepository.findOneOrFail({ id: guildId });
    console.log(
      `[DEBUG] createChannel - Guild found: ${guild.id}, name: ${guild.name}`
    );
    ...
    console.log(
      `[DEBUG] createChannel - About to persist channel: ${channel.name}`
    );
    await this.channelRepository.persistAndFlush(channel);
    console.log(
      `[DEBUG] createChannel - Channel persisted with ID: ${channel.id}`
    );
